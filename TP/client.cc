...
bool interpret_errors(int err_code, int try_nb=0){
	switch(err_code){
		...
		default: printf("< C > This error is not defined: %d",err_code);return false;
	}

	if(try_nb<=MAX_NB_OF_ATTEMPTS)
		printf("< C > Retrying in %d secs...\n",TIME_TO_WAIT);
	else{
		printf("< C > MAX_NB_OF_ATTEMPTS(%d) reached!\nClosing...",MAX_NB_OF_ATTEMPTS);
		return false;
	}
	sleep(TIME_TO_WAIT);
	return true;
...
extern "C" const char* call_client(char* hostname, int client_port, char *client_msg){
	...
	while(e){
		try{			
			...
			printf("< C > Default exception...Closing!");
			bzero((char*)to_return,sizeof(to_return));
			strcpy(to_return,"error");
		}
	}
